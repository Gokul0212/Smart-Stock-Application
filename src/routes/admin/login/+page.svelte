<script>
  import { login, logout } from '$lib/auth.js';
  import { goto } from '$app/navigation';

  let emailOrUsername = $state('');
  let password = $state('');
  let errorMsg = $state('');
  let loading = $state(false);

  async function handleAdminLogin(e) {
    e.preventDefault();
    if (!emailOrUsername || !password) {
      errorMsg = "Please enter both fields.";
      return;
    }

    loading = true;
    errorMsg = '';

    try {
      const user = await login(emailOrUsername, password);
      
      if (user.role !== 'admin') {
        // Log out standard user attempting to access admin
        logout();
        throw new Error("Access Denied: You do not have administrator privileges.");
      }
      
      goto('/admin/dashboard');
    } catch (err) {
      errorMsg = err.message || "Invalid credentials.";
      loading = false;
    }
  }
</script>

<div class="auth-page auth-bg-overlay-admin">
  <div class="auth-card-wrapper">
    <div class="brand" onclick={() => goto('/')}>
      <span class="brand-smart">SMART</span><span class="brand-stock">STOCK</span>
    </div>
    
    <div class="auth-card">
      <div class="admin-header">
        <span class="admin-badge">ADMIN CONTROL</span>
        <h2>System Sign In</h2>
      </div>

      {#if errorMsg}
        <div class="error-banner">
          <span class="error-icon">⚠️</span>
          <span class="error-text">{errorMsg}</span>
        </div>
      {/if}

      <form onsubmit={handleAdminLogin} class="auth-form">
        <div class="form-group">
          <input 
            type="text" 
            bind:value={emailOrUsername} 
            placeholder="Admin Username or Email" 
            required 
            class="auth-input"
          />
        </div>

        <div class="form-group">
          <input 
            type="password" 
            bind:value={password} 
            placeholder="Password" 
            required 
            class="auth-input"
          />
        </div>

        <button type="submit" disabled={loading} class="auth-btn">
          {#if loading}Verifying Credentials...{:else}Sign In to Dashboard{/if}
        </button>
      </form>

      <div class="auth-footer">
        <p>New Administrator? <span class="link-text" onclick={() => goto('/admin/register')}>Create an account</span></p>
        <p class="customer-switch" onclick={() => goto('/login')}>Go to Customer Portal</p>
        <p class="home-link" onclick={() => goto('/')}>← Back to Home</p>
      </div>
    </div>
  </div>
</div>

<style>
  .auth-page {
    min-height: 100vh;
    display: flex;
    justify-content: center;
    align-items: center;
    font-family: 'Inter', system-ui, sans-serif;
    padding: 20px;
    box-sizing: border-box;
  }

  .auth-card-wrapper {
    width: 100%;
    max-width: 450px;
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 20px;
  }

  .brand {
    font-size: 32px;
    font-weight: 900;
    cursor: pointer;
    letter-spacing: 1px;
    user-select: none;
  }

  .brand-smart {
    color: #ffffff;
  }

  .brand-stock {
    color: #e50914;
    text-shadow: 0 0 10px rgba(229, 9, 20, 0.4);
  }

  .auth-card {
    width: 100%;
    background-color: rgba(10, 10, 10, 0.94);
    border: 1px solid #c9252c; /* Subtle red border highlight for admin login */
    border-radius: 8px;
    padding: 50px 40px;
    box-sizing: border-box;
    box-shadow: 0 15px 30px rgba(0, 0, 0, 0.9), 0 0 25px rgba(229, 9, 20, 0.1);
  }

  .admin-header {
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 10px;
    margin-bottom: 25px;
  }

  .admin-badge {
    background-color: #e50914;
    color: #ffffff;
    font-size: 10px;
    font-weight: 850;
    letter-spacing: 1.5px;
    padding: 3px 8px;
    border-radius: 4px;
    box-shadow: 0 0 10px rgba(229, 9, 20, 0.4);
  }

  .auth-card h2 {
    color: #ffffff;
    font-size: 26px;
    font-weight: 700;
    margin: 0;
  }

  .error-banner {
    background-color: rgba(229, 9, 20, 0.15);
    border: 1px solid #e50914;
    border-radius: 4px;
    padding: 12px;
    margin-bottom: 20px;
    display: flex;
    align-items: center;
    gap: 10px;
  }

  .error-icon {
    font-size: 16px;
  }

  .error-text {
    color: #ff8888;
    font-size: 13px;
    font-weight: 500;
  }

  .auth-form {
    display: flex;
    flex-direction: column;
    gap: 16px;
  }

  .auth-input {
    width: 100%;
    background-color: #222222;
    border: 1px solid #333333;
    border-radius: 4px;
    padding: 14px;
    color: #ffffff;
    font-size: 15px;
    box-sizing: border-box;
    transition: all 0.2s;
  }

  .auth-input::placeholder {
    color: #8c8c8c;
  }

  .auth-input:focus {
    background-color: #2a2a2a;
    border-color: #e50914;
  }

  .auth-btn {
    background-color: #e50914;
    color: #ffffff;
    border: none;
    border-radius: 4px;
    padding: 15px;
    font-size: 16px;
    font-weight: 700;
    cursor: pointer;
    transition: background-color 0.2s, box-shadow 0.2s;
    margin-top: 10px;
  }

  .auth-btn:hover:not(:disabled) {
    background-color: #b20710;
    box-shadow: 0 0 15px rgba(229, 9, 20, 0.4);
  }

  .auth-btn:disabled {
    opacity: 0.5;
    cursor: not-allowed;
  }

  .auth-footer {
    margin-top: 30px;
    display: flex;
    flex-direction: column;
    gap: 12px;
    text-align: center;
  }

  .auth-footer p {
    margin: 0;
    color: #737373;
    font-size: 13px;
    font-weight: 500;
  }

  .link-text {
    color: #ffffff;
    cursor: pointer;
    font-weight: 600;
  }

  .link-text:hover {
    text-decoration: underline;
  }

  .customer-switch {
    color: #ffffff !important;
    font-weight: 600 !important;
    cursor: pointer;
  }

  .customer-switch:hover {
    text-decoration: underline;
  }

  .home-link {
    cursor: pointer;
    font-size: 12px !important;
    margin-top: 5px !important;
    display: inline-block;
  }

  .home-link:hover {
    color: #ffffff !important;
  }

  @media (max-width: 480px) {
    .auth-card {
      padding: 30px 20px;
    }
  }
</style>
